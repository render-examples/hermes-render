...
| Dashboard **Chat** tab shows "Chat unavailable: 1" | Upstream bug [NousResearch/hermes-agent#20500](https://github.com/NousResearch/hermes-agent/issues/20500): `/opt/hermes/ui-tui/` ships root-owned in the official image, but the dashboard runs as the unprivileged `hermes` user, so the runtime esbuild rebuild fails with `EACCES`. The Blueprint's `dockerCommand` includes a `chown -R hermes:hermes /opt/hermes/ui-tui` workaround that runs on every container start until upstream fix PR [#19303](https://github.com/NousResearch/hermes-agent/pull/19303) lands. If you've forked the Blueprint and removed it, restore it. |
